...
class KataConcurrentSavesTest < TestBase
  ...
  |   1st call: via file_edit - commits the large file edit
  ...
  | 100 b_threads makes it likely at least one is queued on the mutex when 
  | thread_a releases, making the race reliable.
  |
  ...
  |
  | Confirmed that the test failed before the fix was applied, and passes after.
  ) do
    in_kata do |id|
