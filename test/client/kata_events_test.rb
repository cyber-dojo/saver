...
class KataEventsTest < TestBase
  ...
  | already existing kata_events() summary {test-data copied into saver}
    ...
    expected = [
      { 'index' => 0, 'major_index' => 0, 'minor_index' => '', 'event'  => 'created', 'time' => t0},
      { 'index' => 1, 'major_index' => 1, 'minor_index' => '', 'colour' => 'red',     'time' => t1, 'duration' => 0.491393, 'predicted' => 'none' },
      { 'index' => 2, 'major_index' => 2, 'minor_index' => '', 'colour' => 'amber',   'time' => t2, 'duration' => 0.426736, 'predicted' => 'none' },
      { 'index' => 3, 'major_index' => 3, 'minor_index' => '', 'colour' => 'green',   'time' => t3, 'duration' => 0.438522, 'predicted' => 'none' }
    ]
    actual = kata_events(id)
    assert_equal expected, actual
  end
