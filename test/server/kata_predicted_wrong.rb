...
class KataPredictedWrongTest < TestBase
  ...
  | kata_predicted_wrong stores a predicted-wrong event with correct commit message
    ...
    in_kata do |id|
      ...
      kata_predicted_wrong(id, files, stdout, stderr, status, summary)
  ...
  | on v0/v1 katas
    ...
    assert_raises(NoLongerImplementedError) do
      kata_predicted_wrong(id, files, data['stdout'], data['stderr'], data['status'], summary)
    end
  end
  ...
  | a kata_predicted_wrong event results in two committed events - the pending
  ...
  | that counts the traffic-lights (minor_index 0).
    ...
    in_kata do |id|
      ...
      kata_predicted_wrong(id, files, stdout, stderr, status, red_summary)
