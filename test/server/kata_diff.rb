...
class KataDiffTest < TestBase
  ...
  test 'D4E5A4', %w(
  | v2 kata: empty file is renamed 100% identical
  ) do
    @was_files = { 'plain' => '' }
    @now_files = { 'copy'  => '' }
    assert_diff [
  ...
  test 'D4E5A5', %w(
  | v2 kata: empty file is renamed 100% identical across dirs
  ) do
    @was_files = { 'plain'    => '' }
    @now_files = { 'a/b/copy' => '' }
    assert_diff [
  ...
  test 'D4E5A6', %w(
  | v2 kata: empty file has content added
  ) do
    @was_files = { 'empty.c' => '' }
  ...
  test 'D4E5B4', %w(
  | v2 kata: non-empty file is renamed 100% identical
  ) do
    @was_files = { 'plain' => 'xxx' }
    @now_files = { 'copy' => 'xxx' }
    assert_diff [
  ...
  test 'D4E5B5', %w(
  | v2 kata: non-empty file is renamed 100% identical across dirs
  ) do
  ...
  test 'D4E5B6', %w(
  | v2 kata: non-empty file is renamed less than 100% identical
  ) do
    @was_files = { 'hiker.h'   => "a\nb\nc\nd" }
  ...
  test 'D4E5B7', %w(
  | v2 kata: non-empty file is renamed less than 100% identical across dirs
  ) do
  ...
  test 'D4E5C1', %w(
  | v2 kata: non-empty file has content added at the start
  ) do
  ...
  test 'D4E5C2', %w(
  | v2 kata: non-empty file has content added at the end
  ) do
    @was_files = { 'non-empty.c' => 'something' }
  ...
  test 'D4E5C3', %w(
  | v2 kata: non-empty file has content added in the middle
  ) do
  ...
  test 'D4E5C4', %w(
  | v2 kata: diff_lines with a non-existent now-index raises a reasonable exception
  ) do
  ...
  def assert_diff_lines(expected)
    ...
    result = diff_lines(id, was_index, now_index)
    assert result.include?(expected[0]), diagnostic(expected, result)
  end

  def assert_diff_summary(expected)
    id, was_index, now_index = *run_diff_prepare
    result = diff_summary(id, was_index, now_index)
    assert result.include?(expected[0]), diagnostic(expected, result)
  end

  def diagnostic(expected, actual)
    [
      "#{name}:expected=#{JSON.pretty_generate(expected)}",
      "#{name}:actual=#{JSON.pretty_generate(actual)}"
    ].join("\n")
  end
  ...
  def run_diff_prepare
    ...
    kata_ran_tests(id, @now_files)
    [id, was_index, now_index]
  end

  def starter_manifest
    manifest = kata_manifest('5U2J18')
