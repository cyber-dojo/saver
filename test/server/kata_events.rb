...
class KataEventsTest < TestBase
  ...
  version_test 2, 'ds0', %w( kata_events v2 ) do
    ...
    in_kata do |id|
      ...
      actual = kata_events(id)
      expected = [
        { "index" => 0, "sub-index" => 0, "event" => "created", "time" => t0, "colour" => "create" },
        { "index" => 1, "sub-index" => 0, "time" => t1, "colour" => "red" },
        { "index" => 2, "sub-index" => 0, "time" => t2, "colour" => "red" },
        { "index" => 3, "sub-index" => 0, "time" => t3, "colour" => "red" }
      ]
      assert_equal expected, actual
    end
  end
  ...
  version_test 2, 'ds1', %w( saver outages are recorded as backfilled events ) do
    ...
    in_kata do |id|
      ...
      expected = [
        { "index" => 0, "sub-index" => 0, "event" => "created", "time" => t0, "colour" => "create" },
        { "index" => 1, "sub-index" => 0, "time" => t1, "colour" => "red" },
        { "index" => 2, "sub-index" => 0, "event" => "outage" },
        { "index" => 3, "sub-index" => 0, "time" => t3, "colour" => "red" }
      ]
      assert_equal expected, actual
    end
  ...
  version_test 2, 'eh4', %w( polyfill colour 'create' for index==0 creation event ) do
    ...
    in_kata do |id|
      actual = kata_events(id)
      expected = [{ "index" => 0, "sub-index" => 0, "time" => t0, "colour" => "create", "event" => "created" }]
      assert_equal expected, actual
    end
  end
