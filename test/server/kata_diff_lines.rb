...
class KataDiffLinesTest < TestBase
  ...
  test 'F3D6F1', %w(
  | v0 kata: diff_lines raises NoLongerImplementedError
  ) do
    assert_raises(NoLongerImplementedError) do
      diff_lines(V0_KATA_ID, 0, 1)
    end
  end

  test 'F3D6F2', %w(
  | v1 kata: diff_lines raises NoLongerImplementedError
  ) do
    assert_raises(NoLongerImplementedError) do
      diff_lines(V1_KATA_ID, 0, 1)
    end
  end
